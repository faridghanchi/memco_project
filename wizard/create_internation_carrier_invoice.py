...
class inter_line(osv.osv_memory):

    _name = "inter.line"
    _columns = {
        'product_id': fields.many2one('product.product', 'Product', required=True),
        'price':fields.float(string='Cost price'),
        'inter_carrire_id':fields.many2one('inter.invoice.carrier', 'Inter carrier'),
        
    }
class inter_invoice_carrier(osv.osv_memory):

    _name = "inter.invoice.carrier"
    _columns = {
        'journal_id': fields.many2one('account.journal', 'Destination Journal', required=True),
        'group': fields.boolean("Group by partner"),
        'invoice_date': fields.date('Invoice Date'),
        'partner_id':fields.many2one('res.partner', 'Partner'),
        'line_id':fields.one2many('inter.line','inter_carrire_id',string="Line"),
...
#        'journal_id' : _get_journal,
    ...
    def default_get(self, cr, uid, fields, context=None):
        if context is None:
            context = {}
        res = super(inter_invoice_carrier, self).default_get(cr, uid, fields, context=context)
        active_ids = context.get('active_ids',[])
        print "Activive ids:>", active_ids,fields
        ...
        pickk = []
        print "select_line",select_line.i_picking_id
        for a in select_line.m_cost_line:
            pickk.append((0, 0, {
                    'product_id':a.product.id,
                    'price':a.cost,
                    'inter_carrire_id':active_ids
                }))
        if 'partner_id' in fields:
            res.update({'partner_id': select_line.carrier_company.id})

#        for p in pick.i_carrier_extra_cost:
#            print "P:>>>", p
#            for a in p.m_cost_line:
#                pickk.append((0, 0, {
#                    'product_id':a.product.id,
#                    'price':a.cost,
#                    'inter_carrire_id':active_ids
#                }))
#            if 'partner_id' in fields:
#                res.update({'partner_id': p.carrier_company.id})
        if 'ref' in fields:
            res.update({'ref': select_line.i_picking_id.origin})
...
#          
    ...
    def view_init(self, cr, uid, fields_list, context=None):
        if context is None:
            context = {}
        res = super(inter_invoice_carrier, self).view_init(cr, uid, fields_list, context=context)
        ca_obj = self.pool.get('carrier.extra.cost')
        count = 0
        active_ids = context.get('active_ids',[])
        print "ininit active_ids:>", active_ids
#        for pick in ca_obj.browse(cr, uid, active_ids, context=context):
#            if pick.done_carrier_invoice == True:
